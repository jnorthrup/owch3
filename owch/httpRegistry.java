 ...
 * notified of a
 * waiting pipeline
 * @version $Id: httpRegistry.java,v 1.4 2001/09/23 10:20:10 grrrrr Exp $
...
public class httpRegistry extends Registry {
    ...
    public String displayValue(Reference o) {
        java.util.Map m = (java.util.Map) o.get();
        if (m == null) {
    ...
    protected java.util.Map getWeakMap() { return URLNodeMap; };

    /** references URL prefix-> NodeName */
    private java.util.WeakHashMap URLNodeMap =
        new java.util.WeakHashMap(384);

    /** exported frequently after a change. */
    protected Object[] getCache() { return arrayCache; };

    protected void setCache(Object[] arr) { arrayCache = arr; }
    ...
    public boolean cacheDirty() { return cacheInvalid; };

    public Reference referenceValue(Object o) {
        return new SoftReference(o, refQ());
    };

    public void setDirty(boolean dirt) { cacheInvalid = dirt; };
    ...
    public java.util.Set getSet() { return URLSet; };
    ...
    private java.util.SortedSet URLSet =
    new java.util.TreeSet(getComparator()); //holds the reference
    // to url strings

    /** ************************************************************
     *       dispatchRequest    public boolean
     * dispatchRequest(Socket s,MetaProperties n)
     *
    ...
    public boolean dispatchRequest(Socket s, MetaProperties n) {
        ...
        if (l == null) {
            ...
            for (int i = arrayCache.length - 1; i >= 0; i--) {
                ...
                Env.debug(500, "Pattern test on " + resource +
                    ":" + temp);
                if (temp.length() > len) {
                    continue;
                }
                if (resource.startsWith(temp)) {
                    ...
                    Env.debug(500, "Pattern match on " + resource +
                        ":" + temp);
                }
            }
        }
        if (l != null) {
            ...
            //check to see if the Node that registered this
            ...
            if (Env.getRouter("IPC").hasElement(lname)) {
                ...
                n.put("JMSType", "httpd");
                n.put("JMSReplyTo", "nobody"); //apparently we
            ...
            Env.debug(15, getClass().getName() +
                " creating PipeSocket for " +
                n.get("Resource").toString());
