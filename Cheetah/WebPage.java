...
/** ****************************************************************
 *   base class proof of concept for an object that can
 * start up, suck
 *   down some content, clone, and finally take over
 * functions. this
...
public class WebPage extends Node implements Runnable {
    ...
    public static void main( String[] args ) {
        ...
        m.containsKey( "Resource" ) ) ) {
            ...
            "[-Clone 'host1[ ..hostn]']\n" +
            "[-Deploy 'host1[ ..hostn]']\n" +
            "$Id: WebPage.java,v 1.4 2001/09/23 10:20:10 grrrrr Exp $\n" );
            System.exit( 2 );
        };
    ...
    /** ************************************************************
     *       this is a set of headers that is simply
     * nice to have...
     ************************************************************ */
    private Notification nice = new Notification();

    /** ************************************************************
     *       this is a set of header fields that is simply
     * nice to have...
     ************************************************************ */
    ...
    */
    public WebPage( java.util.Map m ) {
        super( m );
        if ( containsKey( "Source" ) ) {
            init( ( String )get( "JMSReplyTo" ), ( String )get( "Source" ),
            ( String )get( "Resource" ) );
        }
        else {
            init( ( String )get( "JMSReplyTo" ),
            ( String )get( "Resource" ) );
        }
    ...
    */
    ...
    public void inductURL( String url ) {
        try {
            ...
            for ( int i = 0; i < nice_headers.length; i++ ) {
                String hdr = uc.getHeaderField(
                nice_headers[ i ] );
                if ( hdr != null ) {
                    nice.put( nice_headers[ i ],
                    get( nice_headers[ i ] ) );
                };
            };
    ...
    /** ************************************************************
     *       sets the IM HERE interval for our periodic
     * re-registration tasks.
     ************************************************************ */
    ...
    /** ************************************************************
     *       our thread function is to periodically wake up
     * and notify both
     *       the Domain and the Gatekeeper of our existance
     * every so often.
     *       this allows changing of the guard and a
     * roundabout load
    ...
    public void run() {
        while ( !killFlag ) {
            ...
            try {
                long tim = ( long )( Math.random() *
                ( interval / 2.0 ) + ( interval / 2.0 ) );
                Env.debug( 12, getClass().getName() +
                " waiting for " + tim + " ms." );
    ...
    public void sendPayload( Socket s ) {
        ...
        try {
            if ( !nice.containsKey( "Content-Type" ) ) {
                nice.put( "Content-Type",
                "application/octet-stream" );
            };
            if ( !nice.containsKey( "Content-Length" ) &&
            "text/html".intern() !=
            nice.get( "Content-Type" ) ) {
                nice.put( "Content-Length",
                "" + payload.length );
            };
            ...
            getJMSReplyTo() + " failure " + e.getMessage() );
            e.printStackTrace();
        }
    };

    /** ************************************************************
     *       intended for automatic maintenance.. so to speak,
     * for survival
     *       tasks.. period replication, metrics evalutation,
     * etc.  lots o
     *       TBD in this method
     ************************************************************ */
    ...
    public void handle_Move( MetaProperties n ) {
    ...
    public void clone_state1( String host ) {
        ...
        n2.put( "Source", Env.getLocation( "http" ).getURL() +
        get( "Resource" ) );
