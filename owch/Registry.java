...
/** ****************************************************************
 * gatekeeper registers a prefix of an Item such as
 * "/cgi-bin/foo.cgi"
 ...
 *
 * The when an Item is located -- registering the Item
 * "/" is a sure
 * bet, the owch agent registered in the WeakHashMap is
 * notified of a
 * waiting pipeline
 * @version $Id: Registry.java,v 1.3 2001/09/23 10:20:10 grrrrr Exp $
...
public abstract class Registry {
    ...
    protected ReferenceQueue refQ() { return itemQ; }
    ...
    protected void registerItem(Comparable key, Reference val) {
        ...
        Env.debug(15, getClass().getName() +
            ":::Item Registration:" + "@" + displayKey(key) +
            " -- " + displayValue(val));
    };
    ...
    public void unregisterItem(Comparable key) {
        ...
        Env.debug(15, getClass().getName() +
            ":::Item DeRegistration:" + displayKey(key));
    };
    ...
    protected void reCache() {
        Env.debug(150, getClass().getName() +
            " recache starting..");
        synchronized(getSet()) {
