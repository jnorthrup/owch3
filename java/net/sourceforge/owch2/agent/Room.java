...
/**
 * @version $Id: Room.java,v 1.1 2002/12/08 16:05:49 grrrrr Exp $
...
public class Room extends AbstractAgent implements Runnable {
    ...
    public static void main(String[] args) {
        ...
        if (!(m.containsKey("JMSReplyTo"))) {
            Env.cmdLineHelp("\n\n******************** cmdline syntax error\n" + "Room Agent usage:\n\n" + "-name name\n" +
                    "$Id: Room.java,v 1.1 2002/12/08 16:05:49 grrrrr Exp $\n");
        }
        Room d = new Room(m);
        Thread t = new Thread();
        try {
    ...
    synchronized void addUser(MetaProperties clientIn) {
        ...
        if (Env.logDebug) Env.log(8, "Room - addUser = " + userNode + "key = " + userKey);
        if (userKey == null) {
    ...
    public void publish(MetaProperties nIn) {
        ...
        for (Iterator e = usersList.keySet().iterator(); e.hasNext();) {
            ...
            if (Env.logDebug)
                Env.log(8, "Room.Publish (" + roomServerName + ")(" + nIn.getJMSReplyTo() + ") user is " +
    ...
    synchronized public void syncUsers(String nodeNameIn) {
        ...
        if (Env.logDebug) Env.log(8, "Room - syncUsers  ");
        ...
        for (Iterator e = usersList.keySet().iterator(); e.hasNext();) {
            ...
            if (Env.logDebug)
                Env.log(8, "Room.Publish (" + roomServerName + ")(" + notificationOut.getJMSReplyTo() + ") user is" +
    ...
    synchronized final private static void wait120() {
        ...
        try {
            ...
            if (Env.logDebug) Env.log(12, "debug: wait120 Waiting for " + tim + " ms.");
            Thread.currentThread().sleep(tim, 0);
        }
        catch (Exception e) {
        }
        if (Env.logDebug) Env.log(13, "debug: wait120 end");
    }
}
